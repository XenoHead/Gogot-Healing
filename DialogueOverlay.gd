...
@onready var narrative_label: RichTextLabel = $DialoguePanel/MarginContainer/VBoxContainer/NarrativeLabel
@onready var choice_vbox: VBoxContainer = $DialoguePanel/MarginContainer/VBoxContainer/ChoiceVBox
...
func _display_current_node() -> void:
	...
	if branch_choices.size() > 0:
		...
		for choice in branch_choices:
			...
			btn.add_theme_font_override("font", load("res://fonts/Dark Whispers.otf"))
...
func _process_choice_selection(chosen_branch: Dictionary) -> void:
	if GameState.has_method("modify_metrics"):
		GameState.modify_metrics(chosen_branch["action"])
