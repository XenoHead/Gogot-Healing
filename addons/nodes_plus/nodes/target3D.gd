@icon("res://addons/nodes_plus/icons/node_3D/icon_target.png")
class_name Target3D
extends Area3D
## A 3D target/lock-on component.
##
## Scans for potential targets within its area and manages a current target.
## Works with [Follow3D] and [Projectile3D] to provide homing or tracking behavior.
...
func _on_area_entered(area: Area3D) -> void:
	var potential := area.get_parent() as Node3D
	if not potential:
		return
	...
	if debug_mode:
		print("[Target3D] Target entered: ", potential.name)
...
func _on_area_exited(area: Area3D) -> void:
	...
	if debug_mode:
		print("[Target3D] Target exited: ", potential.name)
...
func _select_target(target: Node3D) -> void:
	...
	if debug_mode:
		print("[Target3D] Selected target: ", target.name)
