@icon("res://addons/nodes_plus/icons/node_2D/icon_area_damage.png")
class_name Hitbox2D
extends Area2D
## A 2D Hitbox component.
##
## Detects overlaps with [Hurtbox2D] nodes and provides damage information.
...
func _on_area_entered(area: Area2D) -> void:
	...
	if not damage_node:
		if debug_mode:
			print("[Hitbox2D] Missing Damage node, ignoring hit.")
		return
	if not area is Hurtbox2D:
		if debug_mode:
			print("[Hitbox2D] Area entered is not a Hurtbox2D: ", area.name)
		return

	var hurt: Hurtbox2D = area as Hurtbox2D
	...
	if debug_mode:
		print_rich("[Hitbox2D] Hit [color=green]", hurt.get_parent().name,"'s ", hurt.name, "[/color] for [color=red]", amount, " damage")
