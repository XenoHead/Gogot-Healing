@icon("res://addons/nodes_plus/icons/node_2D/icon_hand.png")
class_name InteractionArea2D
extends Area2D
## A 2D interaction area component.
##
## Detects when a [CharacterBody2D] enters or exits its range and emits signals
...
func get_bodies_in_range() -> Array[Node2D]:
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body not in _bodies_in_range:
		...
		if debug_mode:
			print("[InteractionArea2D] Body entered: ", body.name)
...
func _on_body_exited(body: Node2D) -> void:
	if body in _bodies_in_range:
		...
		if debug_mode:
			print("[InteractionArea2D] Body exited: ", body.name)
...
func _try_interact() -> void:
	...
	# Interact with the nearest body
	...
	if debug_mode:
		print("[InteractionArea2D] Interacted with: ", nearest.name)
